...
from .model_deepmisl_utils import *
...
class MIL_Attention_FC_surv(nn.Module):
    def __init__(self, 
        ...
        self.fusion = fusion
        # self.size_dict_path = {"small": [input_dim, 512, 256], "large": [input_dim, 512, 384]}
        # self.size_dict_omic = {'small': [256, 256]}

        # ### Deep Sets Architecture Construction
        # size = self.size_dict_path[size_arg]
        size = [input_dim, l_model, d_model]
        ...
        self.classifier = nn.Linear(size[2], output_dim)
    ...
    def forward(self, data):
        x_path = data['patch_features']
        mask = data['mask']
        # x_path = x_path[~mask.bool()].unsqueeze(0)
        x_path = x_path[~mask.bool()]
        ...
        # return hazards, S, Y_hat, None, None
        output = {'output': logits.unsqueeze(0), 'attention': A_out}
        return output
