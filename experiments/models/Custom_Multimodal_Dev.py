...
class Custom_Multimodal_Dev(nn.Module):
    def __init__(self, 
                    input_dim=1024, 
        ...
        self.WSI_level_embs_fusion_type = WSI_level_embs_fusion_type

        super(Custom_Multimodal_Dev,self).__init__()
        ...
        # self.wsi_level_encorders = []
        # for wsi_level_emb_size in wsi_level_emb_sizes:
        #     self.wsi_level_encorders.append(LinearProbe(wsi_level_emb_size, inner_dim))
        
        
        if use_WSI_level_embs:

            layers = []
            if WSI_level_encoder_LayerNorm:
                layers.append(nn.LayerNorm(768))

            if WSI_level_encoder_dropout:
                layers.append(nn.Dropout(WSI_level_encoder_dropout))

            for i, [dim_in, dim_out] in enumerate(zip(WSI_level_encoder_sizes[:-1], WSI_level_encoder_sizes[1:])):
                layers.append(nn.Linear(dim_in, dim_out))
                if i != len(WSI_level_encoder_sizes) - 2:
                    layers.append(nn.ReLU())

            # i_dim = 60
            # o_dim = WSI_level_embs_concat_tail_size if WSI_level_embs_fusion_type == "concat" else inner_dim
            # layers.extend([
            #     nn.Dropout(WSI_level_encoder_dropout),
            #     nn.Linear(768, i_dim),
            #     nn.ReLU(),
            #     nn.Linear(i_dim, o_dim),
            # ])

            self.titan_encoder = nn.Sequential(*layers)
            
            if WSI_level_embs_fusion_type == "concat":
                final_layer_input_dim += WSI_level_encoder_sizes[-1]
    ...
    def forward(self, data):
        ...
        if self.use_WSI_level_embs:
            wsi_titans_embs = self.titan_encoder(data['wsi_features'].to(torch.float32))
            # if self.WSI_level_embs_fusion_type == "concat":
            wsi_titans_embs_sum = wsi_titans_embs[0].mean(dim=0)
            # else: # sum case
                # modalities = modalities + [tensor for tensor in wsi_titans_embs.squeeze()]
            if self.WSI_level_embs_fusion_type == "sum":
                modalities.append(wsi_titans_embs_sum.unsqueeze(dim=0))
        ...
        if self.use_WSI_level_embs and self.WSI_level_embs_fusion_type == "concat":
            tail = wsi_titans_embs_sum.unsqueeze(dim=0)
