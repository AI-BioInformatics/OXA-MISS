...
class S4MIL(nn.Module):
    def __init__(self, input_dim, d_model, output_dim, act='gelu', dropout=0, survival = False):
        super(S4MIL, self).__init__()
        self.n_classes = output_dim
        self._fc1 = [nn.Linear(input_dim, d_model)]
        if act.lower() == 'relu':
            self._fc1 += [nn.ReLU()]
        elif act.lower() == 'gelu':
            self._fc1 += [nn.GELU()]
        ...
        self.s4_block = nn.Sequential(nn.LayerNorm(d_model),
                                      S4D(d_model=d_model, d_state=32, transposed=False))
...
if __name__ == "__main__":
    ...
    model = S4MIL(in_dim = 1024, n_classes = 4, act = 'gelu', dropout = 0.25)
    print(model)
