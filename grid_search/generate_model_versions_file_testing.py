...
model_name = "Custom_Multimodal_XA"
csvpath="/work/H2020DeciderFicarra/D2_4/Development/MultimodalDecider/runs_mapping_withseed.csv"
...
changes_template = {
    ...
    'title': ["custom_multimodal_XA_{tumor_type}_kfold_5_all_tissues"], #custom_multimodal_XA_MISSING_MOD_RATE_kfold_5_all_tissues
    ...
    'data_loader.missing_modalities_tables.missing_mod_rate': ["complete","missing_all_60"],#complete
    ...
    # --- K-FOLD SPLIT ---
    'data_loader.KFold.splits': ["/work/H2020DeciderFicarra/D2_4/Development/MultimodalDecider/utils/splits_train_val_test/{tumor_type}"],
    ...
    'missing_modality_test.scenarios': [['genomics_miss_25','complete','genomics_miss_100','wsi_miss_100','genomics_miss_50','genomics_miss_75']],  # Lista vuota o nomi


    # --- MODEL ---
    'model.name': ["Custom_Multimodal_XA"],
    'model.pretrained': [False],
    'model.save_checkpoints': [True],
    # --- MODEL.KWARGS ---
    'model.kwargs.input_dim': [1024],
    ...
    'model.kwargs.inner_dim': [128],
    'model.kwargs.num_latent_queries': [1],
    'model.kwargs.wsi_dropout': [0.0],
    ...
    'scheduler.name': ["MultiStepLR"], #Ovary: OneCycleLR other: MultiStepLR
    ...
    'optimizer.weight_decay': [0.0001], #0.0001 default 0.000001 ovary
...
tumor_types = ["BLCA"]# ["BLCA","BRCA", "KIRC","KIRP","LUSC"] #["BLCA", "BRCA", "COAD", "HNSC", "KIRC","KIRP" ,"LUAD", "LUSC","STAD"]  ### ["PDS"] #, "NACT_primary"]#["BLCA", "BRCA", "COAD", "HNSC", "KIRC","KIRP" ,"LUAD", "LUSC","OV","STAD"] #"BRCA" manca solo seed 44 (running) "LUAD", "KIRP" devo sistemare i fold
...
for tumor in tumor_types:
    ...
    for key, val_list in changes_template.items():
        ...
        for v in val_list:
            if isinstance(v, str):
                resolved.append(v.format(tumor_type=tumor))
            elif isinstance(v, list) and all(isinstance(elem, str) for elem in v):
                resolved.append([elem.format(tumor_type=tumor, dataset=dataset) for elem in v])
            else:
                resolved.append(v)
...
for combo in combinations:
    seed = combo.get('seed')
    tumor_type = combo['title'].split("_")[3]  # Estratto da title es: custom_multimodal_XA_OV_kfold_5_all_tissues
